...
const idProdutoValidoNaResposta = valor => {
    if (valor == null || valor === '') {
        return false
    }
...
const normalizarIdProduto = valor =>
...
const produtoPodeComprar = produto => {
    if (produto == null) {
        return false
    }
            ...
            try {
                ...
                const cat = Array.isArray(listaCategorias)
                    ? listaCategorias.find(
                          c => c.id === produtoApi.categoriaId
                      )
                    : null

                const resMesmaCategoria = await fetch(
                    `${URL_API}/produtos?categoriaId=${encodeURIComponent(
                        produtoApi.categoriaId
                    )}`
                )
                let listaMesma = []
                ...
                const outros = Array.isArray(listaMesma)
                    ? listaMesma.filter(
                          p =>
                              normalizarIdProduto(p.id) !== produtoApi.id
                      )
                    : []
        ...
        return (
            <main className={`container paginaComNav ${styles.pagina}`}>
                <div className={styles.skeletonPagina} aria-busy="true">
                    ...
                    <div>
                        <div
                            className={`${styles.skBloco} ${styles.skTitulo} ${styles.skLinha}`}
                        />
                        <div
                            className={`${styles.skBloco} ${styles.skLinha} ${styles.skLinhaMedia}`}
                        />
                        <div
                            className={`${styles.skBloco} ${styles.skLinha} ${styles.skLinhaCurta}`}
                        />
                        <div
                            className={`${styles.skBloco} ${styles.skPreco} ${styles.skLinha}`}
                        />
                    </div>
                </div>
            </main>
